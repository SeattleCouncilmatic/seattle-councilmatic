...
export default function MuniCodeSection() {
  ...
  // Show in-page jump links when the body is long enough that scrolling
  // past it on a phone is meaningful. Threshold tuned against the corpus
  // distribution: ~74% of summarized sections are <2000 chars and short
  // enough that a jump link is unnecessary chrome.
  const showJumpLinks = !!data.plain_summary && (data.full_text || '').length >= 2000

  return (
    <div className="smc-detail-page">
      <div className="smc-detail-container" id="page-top">
        <Breadcrumb crumbs={[
          { to: '/', label: 'Home' },
        ...
        <header className="smc-detail-header">
          ...
          {showJumpLinks && (
            <a className="smc-summary-jump" href="#plain-summary">Go to Plain Summary ↓</a>
          )}
        </header>
        ...
        <div className={`smc-detail-content${data.plain_summary ? ' smc-detail-content--with-summary' : ''}`}>
          ...
          {data.plain_summary && (
            <aside id="plain-summary" className="smc-summary-panel" aria-label="Plain-language summary">
              ...
              {showJumpLinks && (
                <a className="smc-summary-jump" href="#page-top">Back to top ↑</a>
              )}
              {data.summary_model && (
