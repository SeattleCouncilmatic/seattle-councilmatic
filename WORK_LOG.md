...
- **Color on the legislation summary card.** PR #88 added the indigo CSS but it didn't apply due to a specificity tie with the later-declared `.leg-detail-section` (both single-class). PR #89 attempted a double-class fix that turned *both* the summary and Key changes cards indigo; closed in favor of the narrower scope on the active branch (`frontend/summary-only-indigo`): indigo only on the Plain-language summary card via `.leg-detail-section.leg-card-summary`, Key changes stays white.
