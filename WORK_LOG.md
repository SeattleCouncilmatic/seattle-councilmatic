...
- **LLM output is `{intro, highlights[]}`** (second review round — the dense single paragraph read dry and walled). `intro` = 1-2 sentence local-news-briefing opener sweeping the week's topics (no identifiers); `highlights` = 2-4 bullets, each one item + its newest development, leftovers folded into the last bullet. Rendered full-width above the columns as a real bulleted list. Voice is deliberately between the first two attempts: direct and readable, no bureaucratic stiffness, but no folksy color either ("busy week!", "crossed the finish line" are called out as anti-patterns in the guide). API gotcha discovered here: **output schemas reject `maxItems` entirely and `minItems > 1`** — the 2-4 range lives in the prompt text, not the schema. Old `{intro}`-only payloads still render (the template guards each key).
