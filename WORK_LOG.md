...
- **`DIGESTS_ENABLED` gates acquisition only** (subscribe endpoint 403s; SPA form hides via `signup_open` on the options payload). Confirm/manage/preferences/unsubscribe stay live regardless, so the flag is safe as both the pre-launch gate (default off in prod — merged digest code rides along on unrelated prod deploys without opening signups over SMTP) and a post-launch kill switch that never breaks unsubscribe links already in inboxes. Phase 4 launch = `DIGESTS_ENABLED=true` in the prod `.env`. Default on under `DEBUG` so dev needs no config.
