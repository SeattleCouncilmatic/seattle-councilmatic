...
**Parser quality** (post-fix re-parse 2026-04-26: 7,421 sections + 1 `TitleAppendix` after PR #23 / 81 `ParseValidationIssue` rows)
...
- **Full re-parse to propagate the absorb-loop and ord-citation-boundary fixes.** PR #28 fixed both bugs but only the focused ranges (chapter 21.36 and chapter 23.50) were re-parsed inline. Other chapters with TOC `<section>...<bare-Subchapter>` patterns (i.e. multi-subchapter TOCs) still have empty/truncated subchapter names from the absorb-loop bug. A `--allow-deletes` full-PDF re-parse will rewrite those names AND surface any further validation deltas. Likely drops `ParseValidationIssue` further from the post-PR-#28 baseline of 35.
