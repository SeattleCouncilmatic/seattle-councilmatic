...
- **Fix Legistar bill links (still broken).** PR #74 corrected the host (`legistar.council.seattle.gov` → `seattle.legistar.com`) but the constructed URL (`/LegislationDetail.aspx?ID=<MatterId>`) doesn't reliably resolve without the matching `GUID` query param — Legistar redirects or 404s on most bills. Robust fix: in the bill scraper, capture `MatterInSiteURL` from the API the way `seattle/events.py:171-173` captures `EventInSiteURL`, store it via `bill.add_source()`, expose it on the bill detail JSON as a `legistar_url` field, and have the frontend use that directly instead of constructing a URL from `legistar_id`. Requires a re-scrape of all 381 bills to populate the new sources.
- **Link ordinance references to their target bill.** Bill summaries (and SMC section text) often cite "Ord. 127400" or "Resolution 32168". Today these render as plain text in `key_changes` descriptions and section bodies — same vibe as the `affected_section` SMC links did before PR #83. Make ordinance references real links to `/legislation/<slug-of-the-ordinance-bill>`. Likely path: server-side resolution — the API (or `extract_ordinance_refs` already in `seattle_app/services/ordinance_refs.py`) returns each cite alongside its resolved bill slug; frontend renders a `<Link>` per resolved cite, plain text for unresolved. Mirror the `SmcSectionRef` pattern in `LegislationDetail.jsx`. Touches both `LegislationDetail` (link in `key_changes`/`impact_analysis`) and `MuniCodeSection` (link in section body where ordinance citations appear).

**Frontend polish on legislation summary cards** *(quick)*
- **Color on the legislation summary card.** SMC section pages already render the plain-language summary in a tinted indigo card (`#eef2ff` bg / `#c7d2fe` border / `#3730a3` heading). Bill summaries currently use the default white card chrome — visually unrelated to the SMC treatment, easy to overlook. Apply the same indigo tint to the "Plain-language summary" card on `LegislationDetail` so the LLM-generated content reads consistently across both surfaces.
- **Make `SUMMARY` / `IMPACT` eyebrows pop.** Current treatment (`0.75rem`, `font-weight: 700`, uppercase, `color: #6b7280`) gets lost between the prose paragraphs above and below — readers can't easily see the boundary between Summary and Impact. Bump to higher contrast (a darker color like `#3730a3` to match the indigo card, or `#111827`), maybe a thin underline / bottom border, or a small accent rule on the left. Same change applies to the SMC page's summary panel.

**Accessibility audit**
- **Comprehensive a11y review of the SPA.** Accessibility is foundational for a civic engagement site; the audience explicitly includes residents with disabilities. Run an audit covering: WCAG AA color contrast (the muted gray-on-white meta text is the most likely fail), keyboard navigation across the index pages and detail pages (focus trap on dropdowns, focus rings on links, tab order), screen reader compatibility (semantic heading hierarchy, ARIA labels on icon-only buttons like the navbar hamburger and search clear, `<main>` / `<nav>` landmarks, alt text on images including the homepage hero skyline), skip-links to bypass the header, and form labels (search inputs, filter selects). Tooling: axe-core via the Vite dev plugin or browser extension for automated scan, Lighthouse pass per page, plus manual keyboard + VoiceOver/NVDA walkthrough of the flagship paths (Home → Legislation index → bill detail; Home → Municode index → section detail; address-lookup on /reps/). Document findings, file each remediation as its own issue, and triage by impact.
