...
- **Bills sponsored** *(quick)*. Show a "Bills sponsored" list on `/reps/<slug>/`. The infra is already in place — `legislation_index` supports a `?sponsor=<name>` filter (`api_views.py:222,263-271`), and `RepDetail.jsx:lines 47-93` has the layout slots for an extra section. Likely path: extend `get_rep_by_slug()` (`reps/services.py:432`) — or the rep_detail view — to also return a `sponsored_bills: [{identifier, title, status, slug, date_introduced}, ...]` array filtered by the rep's display name, capped at maybe 20 most-recent; render on `RepDetail` as a card under contact info with each row linking to `/legislation/<slug>`. Mirrors the sponsor → rep direction we already wired up in PR #88.
...
- **Tenure / "serving since".** `Membership.start_date` / `end_date` are already in the Pupa schema — the raw SQL in `reps/services.py:381` queries the `Seattle City Council` org but doesn't pull the dates. Cheap follow-up: include `term_start` and `term_end` on the rep dict, render as "Serving since June 2024" on the rep header. Pairs with relaxing the `is_current` filter to surface former councilmembers under e.g. `/reps/former/` when we want the historical view.
