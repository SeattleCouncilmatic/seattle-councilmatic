...
- ~~**Bills sponsored**~~ *(quick)*. Originally shipped on `reps/bills-sponsored` as a card list. Subsumed into the unified involvement table — see "Bills sponsored + voting history → unified table" under Done.
- ~~**Voting history → roll-call on `LegislationDetail`.**~~ Workstream complete. PR 1: `votes/scraper-and-backfill` (data + 552 VoteEvents / 4,113 PersonVotes); PR 2: `votes/repdetail-surface` (RepDetail recent-votes list, later subsumed); PR 3: `reps/legislation-involvement-table` (unified searchable table on RepDetail joining sponsorship + per-event votes); PR 4: `votes/legislation-rollcall` (per-bill roll call on `LegislationDetail` — full-width section below the body grid, one card per VoteEvent grouped by option, voter names link back to `/reps/<slug>/` for currently-serving members).
