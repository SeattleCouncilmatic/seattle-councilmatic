...
- **Boundary failures on figure-only / layout-only pages.** PR #20's TOC-fold recovery surfaced a smaller class of section losses: body section headings that sit at the top of a page (`L0`) where the *previous* page is a sparse figure/map/table page whose last line is an unfinished ord-citation wrap or a stray label like `'for 23.48.225'`. Examples found in Title 23: `23.48.230`, `23.64.006`, `23.76.067` (each ~1k–8k chars, real sections that the old parser somehow emitted but the post-PR-#20 parser doesn't because `_is_section_boundary(prev_line)` fails on the figure-page tail). The likely fix is per-page boundary reset: treat the start of a new page as a paragraph boundary, OR strip more aggressively on sparse figure pages. Investigate which pages exhibit this and whether a simple "first-line of new page is a boundary" rule causes regressions elsewhere.
