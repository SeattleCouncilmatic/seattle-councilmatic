...
- **LLM output is `{intro, highlights[]}`, conversational** (second review round — the dense single paragraph read dry and walled). `intro` = 1-2 sentence knowledgeable-neighbor opener sweeping the week's topics (no identifiers); `highlights` = 2-4 bullets, each one item + its newest development, leftovers folded into the last bullet. Rendered full-width above the columns as a real bulleted list. API gotcha discovered here: **output schemas reject `maxItems` entirely and `minItems > 1`** — the 2-4 range lives in the prompt text, not the schema. Old `{intro}`-only payloads still render (the template guards each key).
