...
- **Add orphan-section cleanup to the parser.** The parser is currently update-or-create only; sections written by an earlier buggy parse linger as stale rows even after the bug is fixed and the new parse stops emitting them. Caught the catastrophic ghosts (`23.47.004`, `23.54.015`) this way — had to delete them manually after PR #17/#18. Suggested fix: track the set of (title, chapter, section) tuples emitted in a given parse range, then `DELETE FROM seattle_app_municipalcodesection WHERE (title_number, chapter_number, section_number) NOT IN (emitted) AND title_number IN (parsed_titles)` at the end of the run. Gate behind an `--allow-deletes` flag so partial-range parses don't nuke other titles' rows.
...
2. **Multi-line non-hyphen TOC wraps** — `23.47A.040`'s TOC entry wraps to 4 lines without soft hyphens. New `_fold_toc_name_wraps` runs in TOC mode (between `Sections:` marker and the first enumerated body subsection like `A. ` / `1. `), folding every name-continuation line into its preceding section-shaped line.
