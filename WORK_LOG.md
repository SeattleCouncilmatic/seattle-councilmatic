...
- **Link RCW references to apps.leg.wa.gov.** SMC sections and bill text routinely cite state code, e.g. `RCW 35.21.560`, `RCW 36.70A.040`. Today these are plain text. Unlike ordinance refs, RCW sections aren't in our DB so we can't link to internal pages — but the Washington State Legislature publishes every section at `https://app.leg.wa.gov/RCW/default.aspx?cite=<title>.<chapter>.<section>` (and chapter-only at `?cite=<title>.<chapter>`), so we can deep-link out. Detection regex similar to ordinance refs (`\bRCW\s+(\d+[A-Z]?\.\d+(?:\.\d+)?)\b`). Render as an external link with the standard `↗` glyph + `target="_blank" rel="noopener noreferrer"` (consistent with `leg-detail-external-link` treatment). Touches `MuniCodeSection` (section body), `LegislationDetail` (`summary` / `impact_analysis` / `key_changes`), and possibly `EventDetail` once transcripts ship.
