...
**Parser quality** (post-fix re-parse 2026-04-26 after PR #28 + boundary refinements: 7,430 sections + 1 `TitleAppendix` / 31 `ParseValidationIssue` rows / 234 official + 1 synthesized subchapter / 8 declared-but-empty)
- **Recover the last 5 small missing sections** (`12A.14.160`, `23.48.235`, `23.50A.160`, `23.76.067`, `25.24.030`). All ≤ 1k chars, scattered across titles, each with its own layout quirk that PR #22's per-page boundary reset and column-split-strip narrowing didn't catch. Investigate one at a time — likely a mix of column-split title bleed (`8.38.010` style: pdfplumber returns title wraps in wrong order across columns), unfiltered layout artifacts, and individual edge cases. Lowest priority; we're at >99% recovery. (`5.48.050` recovered via PR #28's `Ord. + §` boundary rule — its body had a column-broken Ord. citation as `prev_line`.)
