...
locked decisions, conventions, and a chronological log of meaty work.
**Branch state lives in `gh pr list`; active task tracking lives in `gh issue list` — this file does not duplicate either.**

Filter open work via labels: [`area/*`](https://github.com/SeattleCouncilmatic/seattle-councilmatic/labels?q=area), [`priority/*`](https://github.com/SeattleCouncilmatic/seattle-councilmatic/labels?q=priority), [`type/*`](https://github.com/SeattleCouncilmatic/seattle-councilmatic/labels?q=type). Quick wins surface via [`priority/quick-win`](https://github.com/SeattleCouncilmatic/seattle-councilmatic/issues?q=is%3Aopen+label%3Apriority%2Fquick-win).
...
- **Batch pipeline is drain-then-submit on a 6h cadence (2026-06-04).** The four Anthropic Batch commands (`tag_bill_issue_areas`, `summarize_legislation`, `summarize_events`, `summarize_reps`) poll+persist any in-flight batch and *then* submit a new one in a single invocation — not the old poll-OR-submit toggle. The scheduler runs the full pipeline every 6h (00/06/12/18) with an offset drain pass 1h later (01/07/13/19); all jobs share a `flock`. **New Batch commands subclass `BatchPipelineCommand`** (`seattle_app/services/batch_pipeline.py`) — the shared drain-then-submit machine. State is the `BatchRun` / `PipelineRun` DB models, not the old `data/*_state.json` files (#208), so it survives deploys. The scheduler scripts call the **`run_pipeline` orchestrator** (`manage.py run_pipeline --kind …`), which brackets the *whole* cycle in a `PipelineRun`, records a `PipelineStep` per step (scrape/sync/extract/batch — status, timing, output tail) so a hung scrape is visible, and owns the run lifecycle (#214); batch commands attach via `PIPELINE_RUN_KEY` and don't finish a run they didn't create. Replaced the single 2 AM submit / 3 AM poll, which left results up to ~24h stale when Anthropic ran slow or captions posted after the 2 AM run (#204, #206).
- **Pipeline observability (epic #211, 2026-06).** Built on the drain-then-submit models above: a staff-only Django admin dashboard over `PipelineRun`/`BatchRun`/`PipelineStep` (#209); email health alerts via `check_pipeline_health` — a heartbeat keyed on **full-cycle success** (offset-drains succeed even when the scrape is wedged, so they can't mask it), a recent-failure digest, on its own hourly cron tick *outside* the pipeline flock (so it fires even if a run is stuck), deduped via `PipelineAlertState`, recipients in `PIPELINE_ALERT_EMAILS` / SMTP in `EMAIL_*`, with a `--test` flag (#210); and log hygiene — 30-day `logrotate` (`copytruncate`, so the entrypoint's `tail -f` survives rotation), per-line `ts` timestamps, and a real `LOGGING` config — the project had none (`settings.LOGGING` was `{}`, which is what made pupa KeyError), with its `run_key` filter in a **models-free** module so `dictConfig` doesn't trip `AppRegistryNotReady` (#205). The stack paid for itself mid-build — it surfaced the pupa-scrape regression (#216) and the tag-clobber bug (#217/#218), both of which had been failing silently.

## Active work

→ **[GitHub issues](https://github.com/SeattleCouncilmatic/seattle-councilmatic/issues)**. Migrated 2026-05-04 from this file's "Up next" / "Open threads" sections.

Notable accessibility audit context (kept here because the conventions are applied on every new UI): static pass shipped findings doc (PR #104), then 17 PRs (#105–#124) cleared every priority item plus the runtime issues a Firefox Accessibility Inspector + axe DevTools sweep surfaced on the flagship pages (Home, /legislation, a bill detail, /municode, /reps, /reps/district/N). Conventions to apply on any new UI are captured in `AUDIT_FINDINGS.md` under "Conventions to keep applying" — labels, focus, contrast, headings, landmarks, live regions, document title, Leaflet maps, the two known Firefox false positives.

Parser corpus state (post-fix re-parse 2026-04-26 after `93cb885`): 7,435 sections + 1 `TitleAppendix` / 28 `ParseValidationIssue` rows / 234 official + 1 synthesized subchapter / 8 declared-but-empty.
...
**LLM-derived data lives in a dedicated councilmatic model — never on an OCD scrape-owned field.** The nightly `pupa` scrape re-imports OCD bills/events/people and overwrites their fields every cycle (e.g. `Bill.subject` → `[]`, since the Seattle scraper supplies no subjects), so LLM data on an OCD field is clobbered and re-generated every run — wasted tokens, and it vanishes between cycles. The summary models (`LegislationSummary` / `EventSummary` / `RepSummary`) follow this; issue-area tags broke it (lived on `Bill.subject`, re-tagged every 6h) until moved to the dedicated `BillTags` model (#217/#218). Any new LLM-derived field → create or extend a dedicated model and read from it. (`councilmatic_core.Bill` is multi-table-inheritance over the OCD `Bill` with a shared pk, so a model FK'd to `councilmatic_core.Bill` joins cleanly to OCD `BillSponsorship.bill_id`.)

**`pupa` runs as a subprocess with `DJANGO_SETTINGS_MODULE` dropped.** `run_pipeline` shells out to `pupa update`. pupa's CLI reads `django.conf.settings.LOGGING` and only falls back to its own `pupa.settings` (which has a `handlers` key) when `DJANGO_SETTINGS_MODULE` is unset — but under `manage.py` it's `seattle_app.settings`, whose `LOGGING` lacked `handlers.default`, so pupa `KeyError`ed and aborted every full-cycle silently (#216). The subprocess must pop `DJANGO_SETTINGS_MODULE` from its env (matches the old bash-cron behaviour, where the var was never set).

**Model defaults are canonical in `settings.py`; a `.env` `CLAUDE_*_MODEL` entry is a deliberate exception, never a mirror (2026-07-09).** An env var beats `settings.py`, so a `.env` line that merely duplicates the default silently detaches that environment when the default moves. This bit twice: prod ran `summarize_legislation` on stale `claude-opus-4-7` until 2026-06-03, and the Sonnet 5 rollout audit (2026-07-09) found a leftover `CLAUDE_CODE_SECTION_MODEL=claude-haiku-4-5` pin (the corpus had actually been built on Sonnet 4.6). Policy: keep prod `.env` free of model entries; to change models, change the committed default and deploy. When a pin is genuinely needed (temporary experiment, cost stopgap), comment WHY next to it and remove it when done. Verify with `grep CLAUDE_ .env` on the box before any model rollout.

**Committees live only in OCD `Organization` (no slug) and join to bills/events by *normalized* name, not exact match (#199).** `councilmatic_core.Organization` is empty — the 9 standing committees are `opencivicdata.core.Organization` rows with `classification='committee'`. Organization has **no slug column**, so committee-page slugs are generated with `slugify(org.name)` and resolved by slugifying each committee and matching (the set is ~9 — cheaper than a stored slug). Critically, the three name sources disagree: `Organization.name` uses `&` and no suffix (`"Land Use & Sustainability"`), but a bill's `extras['MatterBodyName']`, an `Event.name`, and a vote event's `extras['event_body_name']` use `and` + a trailing `"Committee"` (`"Land Use and Sustainability Committee"`). So linking a bill/event to a committee runs through `_normalize_committee_name()` (lowercase, `&`→`and`, strip punctuation, drop trailing "committee") on both sides — never `==`. Historical/select committee names (`"Land Use Committee"`, `"Select Budget Committee"`) normalize to a key no current committee matches and simply don't link, which is correct.

**A bill's committee comes from its committee VOTE event, not `MatterBodyName`.** The scraper attributes every bill *action* to the legislature (`Seattle City Council`), and `MatterBodyName` is the body *currently* holding the matter — which is `"City Clerk"` for nearly every bill once it advances past committee (only the 1–2 bills actively in committee name a committee there). The committee that actually *considered* a bill is recoverable only from its committee vote event's `event_body_name` (`VoteEvent.extras`), which covers the bills inside the vote-scrape window (~55 of 387, the recent ones; older bills have no committee signal at all and show no committee link). So `_bill_committee_link()` resolves a bill's committee as: MatterBodyName-if-committee → else most-recent committee vote event → else None; and a committee page's "bills considered" list unions `MatterBodyName` matches with `votes__extras__event_body_name` matches. Matching committee pages' bill lists on `MatterBodyName` alone leaves them near-empty (#199 follow-up).
...
`compose_digests` can submit one Anthropic Batch request per non-quiet digest (an intro paragraph personalized to the subscriber's preferences and this week's matched items); `send_digest_batches` polls the batch, persists `{intro}` to `DigestSend.llm_payload`, and the template renders it. **The AI intro is OFF by default (`DIGEST_LLM_BACKEND=none`) as of the launch decision — see the boilerplate bullet below.** The machinery below is all still live behind the flag. Non-obvious decisions:

- **Launch ships a deterministic boilerplate intro; the AI intro is shelved behind the flag.** After the AI intro reached a good bar (Sonnet editorial, see below) the call was made that it wasn't worth the per-send cost + tuning treadmill for launch. So `DIGEST_LLM_BACKEND` defaults to `none`, and `send_digest_batches._boilerplate_intro` renders a standard two-paragraph opener lightly personalized from the digest's own counts ("Here's your weekly roundup… Below you'll find 8 bills with recent council action and 1 meeting recap. 5 upcoming meetings… in the sidebar."). Quiet weeks skip it (the template's quiet message stands). Flipping `DIGEST_LLM_BACKEND=anthropic` brings the AI intro back with zero other changes — the whole batch path, prompt, and exemplars are intact.
- **Full City Council meetings are included for every subscriber**, in both the recap section and the "Coming up" sidebar, independent of committee membership (`personalization._meeting_reasons`, shared by both paths, matches the normalized name "city council" → reason "Full City Council meeting"). The meeting match was committee-membership-based, so the full council — where final votes happen, relevant to everyone — was structurally excluded from both surfaces (a real gap: a *summarized* City Council meeting simply never appeared).
- **Section order: recaps before legislation.** Recent council activity (meeting recaps) is the timely, dynamic part, so it leads; the bill list follows as the deeper reference. Sections are titled "Recent council activity" and "Key legislation", and the boilerplate intro mirrors the order ("…recent council activity (N recaps), followed by updates on key legislation (M bills)"). The card header folds the bill number into the first title clause; the **second clause (the operative substance — "authorizing … to grant an easement …") renders bold**, since the first clause is often generic boilerplate ("An ordinance relating to the City Light Department").
...
- **`DigestLLMClient` interface** (`digests/services/llm_client.py`), dispatched on `DIGEST_LLM_BACKEND`: `none` (the default now — boilerplate intro), `anthropic` (Batch-API editorial intro), `openai` (deliberately unimplemented stub for the plan's provider-hosted-OSS future). Mirrors the `DigestEmailClient` pattern.
...
- **Intro prompt is bootstrap-calibrated, not hand-tuned** (pre-merge review round). `manage.py bootstrap_digest_intros --since <date>` builds synthetic preference profiles from live data (duck-typed prefs shim — no Subscriber rows) and runs them through the SAME `build_intro_request` with the bootstrap Opus model, one sync call each; the curated best became the few-shot exemplars embedded in `INTRO_SYSTEM_PROMPT`. Regenerate + re-curate there when the voice needs adjusting. The Opus round also exposed a real failure mode now guarded in the prompt: the model merged the anonymous "your district's councilmember" match reason with a *named* followed rep — the guide now forbids inferring identities the input doesn't state.
- **The intro is an editorial opening note on Sonnet** (fifth review round; supersedes the bullet-highlights structure from the second). Haiku failed the quality gate twice — "thin and forced" was the verdict on both the dense-paragraph and the greeting+bullets structures — so `CLAUDE_DIGEST_MODEL` defaults to `claude-sonnet-5` (batched ~$0.006/send vs Haiku's ~$0.001; immaterial at beta scale, revisit if volume makes the line item matter) with adaptive thinking at `effort: low` (same `_supports_adaptive_thinking` gate as the summarizers, so a Haiku override still works). Output is `{intro}` again but redefined: 2-4 short connected paragraphs (80-160 words, blank-line separated) modeled on newsletter editor's notes — find the through-line in the items, connect them, close on what happens next; identifiers inline and sparing; the cards below carry the inventory so the intro must NOT. Rendered as separate styled `<p>`s (split in the send command — `linebreaks` markup can't carry email-safe inline styles). API gotcha kept for the record: **output schemas reject `maxItems` entirely and `minItems > 1`**.
- **Followed councilmembers carry seat labels into the LLM context** (`_followed_with_seats`: "District 5" vs "Position 8 (citywide)"), and the guide only allows "your … councilmember" framing when the seat equals the subscriber's own district — citywide members are included and named but never claimed ("your three councilmembers had a busy week" was the third-round complaint).
- **Subscription model is district + topics** (fourth review round). The subscribe form requires a district and no longer offers councilmember or bill pickers; the district maps to a *representative set* server-side — the district's seat holder plus the citywide Position members, mirroring the council map's "who represents me?" — and `personalization._representatives_of` matches ANY sponsorship by that set with per-person named reasons ("Sponsored by Joy Hollingsworth, your district's councilmember" / "Sponsored by Alexis Mercedes Rinck (citywide)"). This superseded the Phase-2 "primary-only for the district dimension" rule. Those reason strings feed the LLM verbatim, so the model never guesses who "your district's councilmember" is; the guide forbids "bills you follow" outright (nothing can create bill follows). `followed_reps`/`followed_bills` stay on the model and legacy rows still match, but no API path writes them (`options` no longer serves a reps list; the API silently ignores the old keys). Daily cadence is grayed out in both forms (explicit disabled colors per AUDIT_FINDINGS, "(coming soon)") and the API forces `daily_enabled=False` + rejects `weekly_enabled=False` until the daily rollout.
- **digests/0004 gives the orphaned `headshot` column a DB default.** Upstream dropped the field from its model but our 0053-pinned schema keeps the column NOT NULL, and django-councilmatic's Person-mirror signal can't populate it — so ANY Person insert violated the constraint (tests hit it first; the scrape path writes the table via raw SQL in `sync_councilmatic`, which is why prod never noticed). Guarded RunSQL in *digests* (a councilmatic_core migration would recreate the phantom-0054 hazard, #187); replaces the per-call ALTER the test fixtures briefly used, which broke on Postgres "pending trigger events" the moment a test created two people.
- **Topic-tag pills on bill cards**: every `BillTags` tag renders as a pill — matched ones indigo (`#e0e7ff`, the "why you're seeing this"), the rest gray (`#f3f4f6`, topic context) — replacing the "Tagged X" sentence pill; non-tag reasons (sponsored-by, followed) stay indigo sentence pills. The matched/unmatched split is recovered from the snapshot's reason strings at render time (`items_from_snapshot` has no prefs in scope), so the snapshot format didn't change.
- **"Coming up" sidebar** — next week's meetings of committees the subscriber's reps sit on (`personalization.upcoming_meetings`, cancelled excluded, 8-day horizon, cap 5). Computed fresh at SEND time, deliberately NOT snapshotted: forward-looking content must not go stale between compose and send, and quiet-week digests render it too — it's what makes a quiet week worth opening. Not fed to the LLM intro (the intro is about news, not the calendar).
- **Email visuals mirror the SPA** (navy `#2E3D5B` header band, white cards with 2px `#e5e7eb` borders + 8px radius, monospace uppercase identifiers, `#e0e7ff`/`#3730a3` reason pills, system font stack). Layout is fluid-hybrid two-column: main + sidebar sit side by side at 600px and stack on narrow clients with **no media queries** (many email clients strip them); Outlook's Word renderer stacks the columns — accepted degradation.
...
- **Pending `DigestSend` rows ARE the compose→send handoff.** `compose_digests` runs the match and creates `DigestSend(status=pending)` with the `matched_item_ids` snapshot; `send_digest_batches` renders from the snapshot and flips to `sent`/`failed`. The plan's JSON state files predate the #208 state-lives-in-the-DB refactor — don't reintroduce them. Phase 3 slots the Anthropic Batch submit/poll between the two commands; the `intro` template slot already renders from `llm_payload` (regression-tested).
- **Snapshot stores `{type, id, reasons}`, never content.** Titles/summaries/dates are re-fetched by id at render time (and by the future feed page); `reasons` are compose-time match explanations a re-run might not reproduce. Items whose row vanished (re-scrape dropped a bill) are silently dropped and `item_count` records what actually rendered.
- **Match dimensions (UNIONed, each contributing a reason):** BillTags overlap with the subscriber's issue areas; any sponsorship by a followed rep; *primary* sponsorship by the district's councilmember ("At Large" district = the Position 8/9 seats); followed bills. "News" = a `BillAction` in the window (`Max(actions__date)`, lexicographic ISO-string compare — same idiom as api_views). Meeting recaps join through people: `EventSummary` rows for committees a followed or district rep sits on, matched via `_normalize_committee_name` (imported from api_views, not forked — normalization drift would silently drop matches).
- **RepSummary "updates" are deliberately NOT an item type.** The weekly rep refresh bumps `generated_at` (`auto_now`) for every rep whether or not anything changed, so it can't distinguish news from regeneration — every digest would carry every followed rep's card as noise. Rep activity reaches the digest through the sponsorship dimension instead.
- **Weekly window is 8 days, not 7** — the cron fires Sunday 6 AM against date-only action strings, so a 7-day cutoff drops last Sunday's actions on the boundary. Daily = since `last_sent_at`. Weekly zero-match still sends the quiet-week template (the cadence is a promise); daily zero-match skips. Dedup: one send per cadence per day, and a pending row of any age blocks re-compose (a stale pending would double-send when send catches up); `failed` rows don't block, so a same-day retry after a fix is possible.
- **SMTP guard:** outside `DEBUG`, `send_digest_batches` refuses the SMTP transport unless `--allow-smtp` — checked only when pending rows exist, so the pre-Phase-4 prod cron (no subscribers) exits quietly instead of erroring every Sunday. The weekly cron entry ships now (Sunday 6 AM, its own `flock`, logs to sync.log for the existing logrotate); the daily cadence stays off in v1.
- **RFC 8058 one-click headers ship from day one** (`List-Unsubscribe` + `List-Unsubscribe-Post` on every digest via the client's `headers` param) so the header path is exercised over SMTP long before Postmark. Hard bounces (`SMTPRecipientsRefused`) flip the subscriber to `bounced`; other send failures only mark the row. `DigestSend.error` is email-redacted **before** storing (the exception embeds the recipient, and the field renders in the admin) via the new public `redact_emails()` in `seattle_app.logging_filters`.
- **Links are built from `DIGEST_SITE_BASE_URL`** (new setting; dev default `http://localhost:5173`) because cron sends have no request for `build_absolute_uri` — prod must set the public origin before Phase 4 (launch checklist in DEPLOY.md).
- **Test-infra first:** these are the repo's first tests that build Bill/Event/Person fixtures (`digests/tests/fixtures.py`). Gotcha: our schema is pinned at `councilmatic_core.0053`, so the test DB keeps upstream-dropped NOT NULL columns (`headshot`) that django-councilmatic's Person-mirror signal can't populate — the fixture sets a DB-level default. `--since YYYY-MM-DD` on compose is the QA knob for dev DBs whose last scrape predates the window.

### Digests — Phase 1: subscription plumbing + SMTP test transport — 2026-07-09 (#231)

New `digests` app shipping the first of four phases of personalized email digests (weekly council-activity digest, later an LLM intro; full plan lives locally in `.claude/plans/`). Non-obvious decisions:

- **Transport abstraction from day one.** All outbound mail goes through `DigestEmailClient` (`digests/services/email_client.py`), selected by `DIGEST_EMAIL_BACKEND`. Phase 1 ships only the SMTP implementation (reuses the pipeline-alert `EMAIL_*` config) so the whole content pipeline can be QA'd to your own inbox before any Postmark/DMARC work exists. **SMTP is test-to-self only — never real subscribers** (no bounce handling, relay caps). Postmark lands in Phase 4 as a pure transport flip.
- **Signup gate lives in the admin, not the environment.** The `DigestConfig` singleton (pk=1, same pattern as `PipelineAlertState`) carries `signups_enabled`, toggled via the "Digest configuration" checkbox in `/admin/` — deliberately NOT an env var (per the model-defaults convention: no split-brain config), and future runtime digest flags (Phase 5's `DIGEST_INCLUDE_BLURBS`) belong on this row too. It gates acquisition only (subscribe endpoint 403s; SPA form hides via `signup_open` on the options payload, which is `never_cache`d so a flip beats prod's 10-min page cache). Confirm/manage/preferences/unsubscribe stay live regardless, so it's both the pre-launch gate (seeded closed when `DEBUG=False`, so merged digest code rides along on prod deploys without opening signups over SMTP) and a post-launch kill switch that never breaks unsubscribe links already in inboxes. Phase 4 launch = tick the box. Seeds open in dev.
- **Stateless HMAC tokens, purpose-bound.** Manage/unsubscribe links are `"<id>.<HMAC-SHA256(purpose:id:version)>"` keyed by `SUBSCRIBER_TOKEN_SECRET` (falls back to `SECRET_KEY` in dev). Purpose is inside the MAC so an unsubscribe token can't drive the preferences API. Bumping `unsubscribe_token_version` revokes a user's outstanding links. The one-shot email *verification* token is separate machinery (random, stored, cleared on use) because it must be single-use.
- **Manage link → session, not token-in-SPA.** `/digests/manage?token=` trades the HMAC token for a 1-hour session cookie and redirects to the React `/digests/preferences` page, keeping tokens out of SPA URL history. The preferences API is session + CSRF; subscribe and unsubscribe POST are `csrf_exempt` (public endpoint / RFC 8058 one-click POSTs can't carry tokens — the HMAC is the auth). Sessions renew on a sliding window (every authenticated call resets the hour), and self-service recovery exists at `POST /api/digests/manage-link`: an active subscriber gets a fresh manage link emailed, a pending one gets their verification re-sent, anything else silently no-ops behind the same always-202 enumeration guard as subscribe. The preferences page's unauthenticated state renders that request form.
- **PII discipline:** raw emails never logged (`EmailRedactionFilter` on the console handler masks message text AND rendered tracebacks — the traceback path was a real leak caught in the first dev-SMTP smoke test: `SMTPRecipientsRefused` embeds the recipient); `__str__`/admin/command output use `subscriber.id` with masked-email displays; `purge_unsubscribed` hard-deletes 30 days after unsubscribe; unsubscribe page offers immediate deletion.
- **`DigestSend` snapshots (`matched_item_ids`, `llm_payload`) exist now** even though nothing writes them until Phases 2-3 — later phases drop in without a migration.
- **Migration dep pinned to `councilmatic_core.0053`.** Running `makemigrations` regenerates a phantom `0054` inside site-packages from upstream django-councilmatic model/migration drift (biography/headshot removals); depending on it would break every env that didn't locally generate it — same failure mode as #187. If you run `makemigrations` and it emits a councilmatic_core migration, delete it from site-packages and keep the dep on 0053.
- **Vite dev proxy nuances:** `/api/digests` and the three Django token pages (`/digests/confirm|manage|unsubscribe`) proxy with `changeOrigin: false` so `build_absolute_uri` in the verification email points at the browser's host (localhost:5173), not the Docker-internal `app:8000`. SPA-owned `/digests/subscribe` and `/digests/preferences` are deliberately not proxied.
- Two rate limits on subscribe via `django-ratelimit` (5/h/IP, 1/h/email-hash; key is the SHA-256 of the address so raw emails stay out of cache keys) + a honeypot field. No-op under dev's DummyCache; enforced on prod's DatabaseCache.
...
**Vocabulary cross-checked against current committees.** First draft was 11 tags from generic municipal-policy buckets; cross-referencing the 9 council committees ("Housing, Arts, & Civil Rights", "Parks & City Light, Arts & Culture", "Transportation, Waterfront, & Seattle Center", etc.) surfaced 9 missing tags — Waterfront, Seattle Center, Parks & Public Spaces, Utilities, Education & Libraries, Governance, Elections, Neighborhoods & Community, Tribal Relations. Final list of 20 in `claude_service.BILL_TAG_VOCABULARY`. Lesson worth keeping: vocabulary design for a city-specific corpus should start from the actual committee structure, not generic municipal-policy categories — committees signal what topics get legislated on.

**Two API gotchas worth recording.**

- **`output_config.format.schema` rejects `minItems`/`maxItems` on arrays.** First smoke batch errored 3/3 with `"For 'array' type, property 'maxItems' is not supported"`. Existing `LEGISLATION_OUTPUT_SCHEMA` had quietly avoided these, just describing cardinality in the field's `description` and relying on the prompt + post-processing to enforce. Fixed by dropping both, adding a hard 3-tag cap in `_process_results`, and noting the constraint in the schema's description so the next person doesn't add them back.
- **One transient `OverloadedError` in 372 requests.** "API key validation is temporarily unavailable. Please retry." Re-submitting the single bill via `--bill` worked first try. Worth knowing the Batch API can surface this as a per-request error rather than failing the whole batch — handle by retry, not by treating as data quality.

**Budget & Taxes over-tagging fixed by prompt tightening.** First-pass Sonnet attached "Budget & Taxes" as a secondary tag to 51% of bills (198/387), because contract authorizations, fee adjustments, and routine appropriations all involve money. For per-rep aggregation that meant every councilmember would look like a Budget & Taxes specialist. Tightened the prompt to reserve "Budget & Taxes" for bills that are *substantively about* budget allocation, taxation, levies, ballot measures funding city programs, or fiscal policy — not bills where money is incidentally a mechanism. Re-tag with `--force` dropped Budget & Taxes to 154/387 (40%); 1-tag bills increased 175 → 203 (more concise tagging); 3-tag bills decreased 54 → 46 (less over-tagging). Stress-sample regressions were minimal: CB 120985 (Comprehensive Plan) lost "Climate & Environment" as a third tag, which is borderline-defensible since the comp plan does drive climate goals. Net positive — substantive topic now leads.

**`--bills` flag for curated stress sampling.** Added a comma-separated bill identifier flag separate from `--bill` (single). Used during the smoke phase to feed the tagger 12 bills selected to span the vocabulary's diverse buckets (transit, housing, surveillance, comp plan, hydropower-with-tribal-settlement, etc.) so we could sanity-check tag quality across topic edges before re-tagging at scale. Worth keeping for future re-runs when prompt or vocabulary changes need targeted before/after comparison.

**Use of bill body text.** Input is `title + first 2 KB of BillText.text` (when available). Title alone gets you 80% of the signal — "Seattle Public Utilities" → Utilities, "City Light Department" → Utilities, "Arts and Cultural District" → Arts & Culture. The 2 KB body excerpt mattered most on bills with generic procedural titles where the substantive topic hides in the body. Best example: CB 121177 ("ordinance relating to the City Light Department; authorizing… Comprehensive Settlement Agreement, five Off-License Agreements, and other related agreements for…") — title alone reads as Utilities, but the body context flagged it as a hydropower relicensing tribal settlement, and the tagger correctly added Tribal Relations.
...
**API** — `legislation_detail` adds `'votes__votes'` and `'votes__counts'` to its prefetch and ships a new `roll_call` array. Voter-name → councilmatic slug resolution batches across every PersonVote on every event into one SQL JOIN (same pattern as the existing sponsor lookup), filtered by `cp.is_current = TRUE` so links never go to a 404 — Sara Nelson and Mark Solomon are in `councilmatic_core_person` but with `is_current=FALSE`, so they fall out of the slug map alongside the historical members (Cathy Moore, Tammy J. Morales, Tanya Woo) who aren't in the table at all. Shape per event: `{date, body_name, is_council, motion_text, result, counts: {opt: n, ...}, votes_by_option: {opt: [{name, slug}, ...], ...}}`.

**Frontend** — new `RollCall.jsx` + `.css`. Section renders nothing when `roll_call` is empty (e.g., bills introduced before the 548-day vote scrape window). Per-event card layout: header with body name + date + a "Full council" tag for the City Council vote; tally line with pass/fail color cue + "8–0" headline + parenthesized non-decisive counts; per-option `<dl>` with chip + count + comma-separated voter names. Motion text is shown small + italic at the bottom of the card so the boilerplate "The Motion carried…" prose is available without competing with the breakdown. Same chip palette as the involvement table for visual consistency.

3+ vote outliers (5 bills today: budget bills, comp-plan rewrites, algorithmic-rent CB) get a card per event — no truncation. With most bills hitting 1-2 events, the section is short by default.

### Reps — unified Legislation involvement table on RepDetail — committed 2026-05-03

Replaces the separate "Bills sponsored" card list and "Voting history" recent-votes list on `RepDetail` with a single searchable, sortable, paginated table that joins both sets of data. One row per bill where the rep has any involvement (sponsored OR voted). Bills the rep neither sponsored nor voted on are excluded so the page stays scoped to their own legislative footprint.

**Why one table over two card lists:** the cards forced a 25-vote inline cap (Hollingsworth has 494 votes — couldn't render them all) and a "View all" link off-page for the rest. A table at 25 rows/page with client-side filter + sort scales cleanly to anyone's footprint and lets users answer power-user questions ("did Saka sponsor AND vote on this bill?") in one place. Bills are the natural spine because sponsorship and voting are both attached to bills.

**Columns:** Bill (just the identifier, e.g. `CB 121200`; full title is on the link's `title` attr as a hover tooltip and visible on the linked bill detail page) / Status (legislation index palette) / Sponsorship (Primary / Cosponsor / —) / Committee vote (option chip + small body name) / Council vote (option chip) / Outcome (Pass / Fail / —). All sortable; default order is bills' `latest_action_date` desc (server-sent). Status differs from Outcome on purpose — a bill can have a passing council vote but still sit in 'Awaiting Mayor's Signature' as its overall MatterStatusName status.

The page container also bumped 72rem → 90rem (matching LegislationDetail) so the 6-column table fits without horizontal scroll on standard desktop widths. The Bill column was originally id + 2-line clamped title which was the main width hog; dropping the inline title to a hover tooltip narrowed the column enough to make the rest of the table fit comfortably.

**Two vote columns, not one:** scraped data shows 98% of voted bills have ≤2 vote events (35% just one, 63% the typical committee→council pair); only 6 bills out of 330 have 3+. Bucketing by `event_body_name` ('City Council' → council column, anything else → committee column) covers 98% cleanly. For 3+ vote bills (budget bills, big zoning packages cycling through multiple committee steps), the most-recent committee vote fills the committee column and a "+N earlier" link sends users to the bill detail page for the full roll-call. `extra_committee_votes` carries the count.

**API** — new `_get_legislation_involvement(person_id, name)` in `reps/services.py`. Three queries: sponsorships (Bill ←via `sponsorships__name__iexact`), per-person votes (PersonVote `voter_id` joined to VoteEvent for body name + result), then a bulk metadata fetch for the union of bill ids. Sponsorship lookup uses `name__iexact` to match the legislation index's sponsor filter; vote lookup uses `voter_id` to dodge the name-collision pitfall. Per-bill bucket merges everything into the row shape the frontend table renders. Hollingsworth → 324 rows / ~75KB JSON / ~200ms server time. Slimmed `_get_voting_history` to just `{total, breakdown}` (the `recent` field is no longer used; replaced by the table). Dropped `_get_sponsored_bills` entirely; `sponsored_bills` and `sponsored_bills_total` no longer ship in the API payload.

**Frontend** — new `LegislationInvolvementTable.jsx` + `.css`. Vanilla React (no table dep added). Search filters case-insensitively on `bill.identifier` and `bill.title`; sort headers use `<button>` inside `<th>` with `aria-sort="ascending"|"descending"|"none"`; pagination as `<nav aria-label="Pagination">` matching the legislation index pattern. Mobile: table wrapper has `overflow-x: auto` + `tabIndex={0}` so it's keyboard-scrollable when content overflows the viewport. The lifetime breakdown pill row (`486 Yes / 4 No / 1 Abstain / ...`) stays as a stats strip above the table for at-a-glance vote pattern; this lives in `RepDetail.jsx` and uses what's left of `voting_history`.

**Removed from `RepDetail.jsx`:** the entire "Bills sponsored" card-list section and the "Voting history → Recent votes" `<ol>`. Removed the dead CSS for `.rep-detail-bills*`, `.rep-detail-vote-recent*`, `.rep-detail-vote-list/row/meta/date/option/result/bill*`, `.rep-detail-vote-truncated`. Kept `.rep-detail-vote-stat*` since the breakdown row still renders.
...
`seattle/people.py` gets a `profile_slug(name)` helper (lowercase + spaces→hyphens) plus a small `PROFILE_SLUG_OVERRIDES` dict for cases where seattle.gov uses a preferred name on the URL — currently just `Robert Kettle → bob-kettle`. Migration `0021_update_council_profile_urls` walks existing `core.PersonLink` rows with `note='City Council profile'` and rewrites the URL using the same rule (no re-scrape required). Verified all 11 stored profiles (9 current + Sara Nelson + Mark Solomon as former) update cleanly.

Caveat: seattle.gov silently redirects former-member URLs to their successor (e.g. `sara-nelson` → Dionne Foster's page, `mark-solomon` → Eddie Lin's page) rather than serving an archived profile. Acceptable for now — the link works and lands on the seat's current holder, which is at least informative.
