...
| `python manage.py summarize_events` | After `extract_event_transcripts` runs against new transcripts; re-run with `--force` after a prompt change or chapter-marker re-extraction | Yes — UPSERTs by `event_id`. Two-phase: submit, wait ~5-10 min, re-run to poll + persist. ~$0.10/meeting |
